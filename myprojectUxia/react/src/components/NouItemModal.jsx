...
const NouItemModal = ({ expo, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      selectedEtiquetes.forEach((id) => formData.append("etiquetes_ids", id));
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-center px-6 py-4 border-b">
          <div>
            <h2 className="text-lg font-bold text-gray-900">+ Nou Ítem</h2>
            <p className="text-xs text-gray-500 mt-0.5">Expo: {expo.nom}</p>
          </div>
          <button
            onClick={onClose}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              <span className="text-gray-400 font-normal text-xs">
                (opcional — si s'afegeixen, l'expo passa a ACTUALITZABLE)
              </span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer border-2 border-dashed border-gray-300 hover:border-blue-400 rounded-lg px-4 py-3 transition">
              <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586A2 2 0 0111.414 11H13a2 2 0 011.414.586L19 16M14 8a2 2 0 11-4 0 2 2 0 014 0z" />
              </svg>
