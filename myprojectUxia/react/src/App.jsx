...
function App() {
  ...
  const seleccionarExpo = async (expo) => {
    try {
      ...
      const response = await fetch(`/api/items?expo_id=${expo.id}`);
  ...
  const verDetalleItem = async (item) => {
    try {
      setLoading(true);
      const response = await fetch(`/api/imatges?item_id=${item.id}`);
  ...
  const anteriorExpo = () => {
    const indexActual = exposiciones.findIndex((e) => e.id === expoActual.id);
    const anteriorIndex =
      (indexActual - 1 + exposiciones.length) % exposiciones.length;
    ...
    <div className="app-container">
      ...
      <main className="main-content">
        ...
        <div className="expo-selector">
          <button className="nav-expo-btn" onClick={anteriorExpo}>
            ◀◀
          </button>
          <h1 className="expo-titulo">{expoActual.nom}</h1>
          <button className="nav-expo-btn" onClick={siguienteExpo}>
            ▶▶
          </button>
          <p className="expo-info">
            {expoActual.lloc} | {expoActual.data_inici} - {expoActual.data_fi}
          </p>
        </div>
          ...
          <>
            <div className="carrousel-container">
              ...
              >
                {itemActual.imatge && (
                  <img
                    src={`/media/${itemActual.imatge}`}
                    alt={itemActual.nom}
                    className="item-imagen"
                  />
                )}
                <h2 className="item-nombre">{itemActual.nom}</h2>
                <p className="item-descripcion">{itemActual.descripcio}</p>
                <p className="item-contador">
                  Item {indiceItem + 1} de {items.length}
                </p>
                <p className="item-click-hint">
                  {" "}
                  Haz click para ver todas las imágenes{" "}
                </p>
              </div>

              {/* Contenedor de las dos flechas juntas */}
              <div className="carrousel-nav">
                <button className="nav-item-btn" onClick={anteriorItem}>
                  ◀
                </button>
                <button className="nav-item-btn" onClick={siguienteItem}>
                  ▶
                </button>
              </div>
            </div>
            <p className="expo-contador">
              Exposición{" "}
              {exposiciones.findIndex((e) => e.id === expoActual.id) + 1} de{" "}
              {exposiciones.length}
            </p>
          </>
        ) : (
      ...
      {itemSeleccionado && (
        <div className="modal-overlay" onClick={cerrarDetalle}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <h2 className="modal-titulo">{itemSeleccionado.nom}</h2>
            <p className="modal-descripcion">{itemSeleccionado.descripcio}</p>
            ...
            {itemSeleccionado.todas_imagenes &&
            itemSeleccionado.todas_imagenes.length > 0 ? (
              <div className="imagenes-grid">
                {itemSeleccionado.todas_imagenes.map((img) => (
                  <div key={img.id} className="imagen-card">
                    <img
                      src={`/media/${img.imatge}`}
                      alt="Imagen del item"
