...
class Command(BaseCommand):
    help = 'Genera una Expo aleatoria con Faker y asigna imágenes aleatoriamente'
    ...
    def handle(self, *args, **options):
        ...
        expo = Expo.objects.create(
            ...
            fecha_fin=fecha_fin,
            lugar=f"Recinto {fake.street_name()}, {ciudad}",
            descripcion=f"Exposición de vehículos en {ciudad}. Ven a descubrir los últimos modelos.",
        ...
        for i in range(num_items):
            ...
            item = Item.objects.create(
                nombre=nombre_item,
                expo=expo,
                descripcion=f"{nombre_item} - {fake.sentence(nb_words=8)}"
            )
            ...
            for nombre_carpeta, nombre_foto in fotos_seleccionadas:
                ...
                try:
                    with open(ruta_foto, 'rb') as f:
                        ...
                        nueva_img.imagen.save(nombre_foto, File(f), save=True)
                        es_primera = False
            ...
            if not expo.imagen and fotos_seleccionadas:
                nombre_carpeta, nombre_foto = fotos_seleccionadas[0]
                ruta_foto = os.path.join(ruta_base, nombre_carpeta, nombre_foto)
                try:
                    with open(ruta_foto, 'rb') as f:
                        expo.imagen.save(nombre_foto, File(f), save=True)
                except Exception as e:
